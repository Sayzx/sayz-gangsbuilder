...
function GetGangs()
	local data = LoadResourceFile('sayz_gangsbuilder', 'data/gangData.json')
...
ESX.RegisterServerCallback('Sayzx-GangsBuilder:Admin_getUsergroup', function(source, cb)
...
;
		...
		SaveResourceFile('sayz_gangsbuilder', 'data/gangData.json', json.encode(GangsData))
	end)
end)
